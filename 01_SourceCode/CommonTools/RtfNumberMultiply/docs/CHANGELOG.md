...
### Bug 修复
- **修复追加无结果 bug**（根因：弹窗隐藏后焦点未切回 WPS，Ctrl+C 复制到聊天框/其他窗口内容而非数字）
- 追加时**轮询检测前台窗口**（`GetForegroundWindow`），确认回到 WPS/Word 后再发 Ctrl+C（最多等 2 秒）
- 首次日志诊断确认的根因：剪贴板读到了 `1.30KN`（旧结果残留）和聊天记录文本
